...
//export FilterArrayBuffer
func FilterArrayBuffer(c *C.char) unsafe.Pointer {
	// 传入 arraybuffer 返回 arraybuffer
	vv := C.GoString(c)
	//vv := string(v)
...
func CountArrayBuffer(c *C.char) C.int {
	// 传入 arraybuffer 返回 int32
	vv := C.GoString(c)
	//vv := string(v)
	fmt.Println("golang out >>>>", vv, len(vv))
	return C.int(len(vv))
...
//export IsArrayBufferType
func IsArrayBufferType(c *C.char) bool {
	// 传入 arraybuffer 返回 boolean
	vv := C.GoString(c)
	//vv := string(v)
	fmt.Println("golang out >>>>", vv, len(vv))
	return len(vv) > 0
...
//export SyncCallbackReArrBuffer
func SyncCallbackReArrBuffer(arg *C.char) unsafe.Pointer {
	// 同步执行，会发生主线程阻塞
	// 传入 arraybuffer，返回 arraybuffer
	ch := make(chan bool)
...
//export ASyncCallbackReArrBuffer
func ASyncCallbackReArrBuffer(arg *C.char, cbFuncStr *C.char) unsafe.Pointer {
	// 异步执行，不会阻塞主线程
	// 传入 string string，返回 arraybuffer
	fmt.Println("golang out >>> cbFuncStr = ", C.GoString(cbFuncStr))
	return SyncCallbackReArrBuffer(arg)
}
...
func ASyncCallbackMArg(arg *C.char, cbFuncStr *C.char, ab *C.char) bool {
	// 异步执行，不会阻塞主线程
	// 传入 string string arraybuffer，返回 boolean
	fmt.Println("golang out >>> cbFuncStr = ", C.GoString(cbFuncStr))
	return SyncCallbackReBool(arg)
}
